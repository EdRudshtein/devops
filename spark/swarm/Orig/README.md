...
```sh
|
|--|apps # Apps directory for volume mounts(any apps you want to deploy just paste it here)
|--|data # Data directory for volume mounts(any file you want to process just paste it here)
|--|Dockerfile-Conda #Dockerfile-Conda used to build spark OLD
...
services:
  ...
  spark-worker-a:
    ...
    environment:
      ...
      - SPARK_LOCAL_IP=spark-worker-age
    volumes:
       - ./apps:/opt/spark-apps
       - ./data:/opt/spark-data
  spark-worker-b:
