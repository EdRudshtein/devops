...
|--|Orig.Ver1.Dockerfile-Conda #Orig.Ver1.Dockerfile-Conda used to build examples image
|--|start-examples.sh # startup script used to run different examples workloads
...
# builder step used to download and configure examples environment
...
# Download and uncompress examples from the apache archive
...
# When the examples work_load is orig.imaging run class org.apache.examples.deploy.orig.imaging.Master
if [ "$SPARK_WORKLOAD" == "master" ];
then

export SPARK_MASTER_HOST=`hostname`

cd /opt/examples/bin && ./examples-class org.apache.examples.deploy.orig.imaging.Master --ip $SPARK_MASTER_HOST --port $SPARK_MASTER_PORT --webui-port $SPARK_MASTER_WEBUI_PORT >> $SPARK_MASTER_LOG

elif [ "$SPARK_WORKLOAD" == "worker" ];
then
# When the examples work_load is worker run class org.apache.examples.deploy.orig.imaging.Worker
cd /opt/examples/bin && ./examples-class org.apache.examples.deploy.worker.Worker --webui-port $SPARK_WORKER_WEBUI_PORT $SPARK_MASTER >> $SPARK_WORKER_LOG
...
```sh
docker build -t cluster-apache-examples:3.0.2 .
```
...
![alt text](examples/articles/images/spark-master.png "Spark Master UI")

Spark Worker 1: http://localhost:9091

![alt text](examples/articles/images/spark-worker-1.png "Spark worker 1")

Spark Worker 2: http://localhost:9092

![alt text](examples/articles/images/spark-worker-2.png "Spark worker 2")
...
```sh
/opt/examples/bin/examples-submit --orig.imaging examples://examples-orig.imaging:7077 \
--jars /opt/examples-apps/postgresql-42.2.22.jar \
--driver-memory 1G \
--executor-memory 1G \
/opt/examples-apps/main.py
```

![alt text](examples/articles/images/pyspark-demo.png "Spark UI with pyspark program running")
...
```sh
/opt/examples/bin/examples-submit --deploy-mode cluster \
--orig.imaging examples://examples-orig.imaging:7077 \
--total-executor-cores 1 \
--class mta.processing.MTAStatisticsApp \
--driver-memory 1G \
--executor-memory 1G \
--jars /opt/examples-apps/postgresql-42.2.22.jar \
--conf examples.driver.extraJavaOptions='-Dconfig-path=/opt/examples-apps/mta.conf' \
--conf examples.executor.extraJavaOptions='-Dconfig-path=/opt/examples-apps/mta.conf' \
/opt/examples-apps/mta-processing.jar
```

You will notice on the spark-ui a driver program and executor program running(In scala we can use deploy-mode cluster)

![alt text](examples/articles/images/stats-app.png "Spark UI with scala program running")
