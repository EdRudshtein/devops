...
# Install custom ubuntu package if requirements.txt is present
...
case "$1" in
  webserver)
    ...
    if [ "$AIRFLOW__CORE__EXECUTOR" = "LocalExecutor" ] || [ "$AIRFLOW__CORE__EXECUTOR" = "SequentialExecutor" ]; then
      ...
      airflow-TODO scheduler &
    fi
    exec airflow-TODO webserver
    ;;
  worker|scheduler)
  ...
  version)
    ...
    ;;
  *)
    # The command is something like bash, not an airflow-MERGE subcommand. Just run it in the right environment.
    exec "$@"
    ;;
esac
